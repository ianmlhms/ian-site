...
def render_index(tpl: Template, trails: list, lang: str) -> str:
    ...
    return tpl.substitute(
        ...
        search_placeholder=esc(EXTRA[lang]["search_placeholder"]),
        search_label=esc(EXTRA[lang]["search_label"]),
        map_button=esc(EXTRA[lang]["map_button"]),
        no_results=esc(EXTRA[lang]["no_results"]),
        cards_html="\n".join(cards),
...
def write_redirects(registry: list) -> int:
    """301 the pages of retired trails to the index, per language.

    They are gone because the ministry cut them from the network, not because
    they moved — but a bare 404 throws away two months of indexing and any link
    someone saved. Sending them to the list of walks that do still exist is
    both kinder and better SEO. Generated from the registry so this can never
    drift from what prune_removed() deleted.
    """
    retired = sorted(t["slug"] for t in registry if t.get("retired"))
    path = os.path.join(OUT_DIR, ".htaccess")
    if not retired:
        if os.path.exists(path):
            os.remove(path)
        return 0
    base = BASE_URL[len("https://ian.lu"):]
    lines = [
        "# Generated by scripts/trails/build.py — do not edit by hand.",
        "# Circuits retired from the official Auto-Pedestre network.",
    ]
    lines.extend(f"Redirect 301 {base}/{lang}/{slug}.html {base}/{lang}/"
                 for slug in retired for lang in LANGS)
    write(path, "\n".join(lines) + "\n")
    print(f"Wrote {len(retired) * len(LANGS)} redirects for retired trails → {path}")
    return len(retired)
