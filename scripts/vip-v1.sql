...
-- seed the owner; add the others below once you have their emails/usernames
insert into public.vip_users (email) values ('konto@ian.lu') on conflict do nothing;

-- Enable VIP for people by USERNAME (fill in the real usernames, then run this):
...
returns boolean language sql security definer stable set search_path = public as $$
  select exists (select 1 from public.vip_users
...
create or replace function public.vip_user_ids()
returns table(user_id uuid) language sql security definer stable set search_path = public as $$
  select u.id from auth.users u
  where lower(u.email) in (select lower(email) from public.vip_users);
$$;
