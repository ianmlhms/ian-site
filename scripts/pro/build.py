...
PRO_SITES_SQL = HERE.parent / "pro-sites-data.sql"
...
def index_hero(data: dict, path: Path) -> tuple[str, str]:
    """Get the hero path and default-language title for the private index."""
    for section in data["sections"]:
        ...
        if not isinstance(image, str):
            break
        title = text(section.get("title"), data, data["defaultLanguage"]) or data["name"]
        if read_image_size(OUT_DIR / data["slug"] / image) is None:
            fail(path, "sections.hero.image", "must be a readable image for the preview index")
        return image, title
    fail(path, "sections", "must include a hero section with an image for the preview index")
    raise AssertionError("fail() always raises")


def sql_literal_lines(value: str) -> list[str]:
    """Quote a value for SQL without letting any line exceed 56 characters."""
    chunks = []
    remaining = value
    while remaining:
        chunk = ""
...
def sql_value_lines(value: str, comma: bool = True) -> list[str]:
    """Return one safely quoted SQL value, split over short lines if needed."""
    values = sql_literal_lines(value)
    lines = ["  " + item for item in values]
...
def write_pro_sites_data(files: list[Path]) -> None:
    """Export preview-index data for the RLS-protected pro_sites table."""
    entries = []
    for path in files:
        try:
        ...
        hero, tagline = index_hero(data, path)
        theme = data.get("theme", {})
        entries.append({
            "slug": data["slug"],
            "name": data["name"],
            "tagline": tagline,
            "town": index_town(data),
            "brand": theme.get("brand", DEFAULT_THEME["brand"]),
            "hero": f"{data['slug']}/{hero}",
            "is_demo": data["slug"] == "demo-bistro",
        })
    ...
    for index, entry in enumerate(entries, 1):
        ...
        for field in ("slug", "name", "tagline", "town", "brand", "hero"):
            lines.extend(sql_value_lines(entry[field]))
        lines.append("  " + ("true" if entry["is_demo"] else "false") + ",")
        lines.extend(sql_value_lines(f"{index:04d}", comma=False))
        lines.extend([
            ")",
    ...
    for index, entry in enumerate(entries):
        lines.extend(sql_value_lines(
            entry["slug"], comma=index < len(entries) - 1,
        ))
    lines.extend([
        "]);",
        "",
    ])
    PRO_SITES_SQL.write_text("\n".join(lines), encoding="utf-8")
    print(f"built scripts/pro-sites-data.sql ({len(entries)} previews)")


def build_index() -> None:
    """Build the empty shell for the database-backed private preview list."""
    (OUT_DIR / "index.html").write_text(
        (TEMPLATE_DIR / "pro-index.html").read_text(encoding="utf-8"),
        encoding="utf-8",
    )
    shutil.copyfile(TEMPLATE_DIR / "pro-index.css", OUT_DIR / "site.css")
    print("built pro/index.html")
