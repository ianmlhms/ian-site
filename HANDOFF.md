...
| **Moien** | `moien.html`, `supabase/functions/transport` | **Luxembourg day companion.** LB greeting + live clock/date, **weather** via Open-Meteo (no key, live now), and **live public-transport departures** via the `transport` Edge Function (proxies the Verkéiersbond/HAFAS `departureBoard`+`location.name`). Transport is **LIVE** — `TRANSPORT_API_KEY` is set and verified (4 Aug 2026: `configured:true`, 8 live departures for Niederanven Laach `id=200504002`). Board action param is `id` (`?action=board&id=<stopId>`). Setup/history: `scripts/MOIEN-SETUP.md`. Public page (no login). |
| **SkyLens** | `skylens.html`, `skylens.css`, `skylens.js`, `skylens.webmanifest`, `supabase/functions/skylens` | **Public live aircraft tracker.** Map-first Leaflet/OSM radar with visible-bounds ADS-B queries (5–250 NM), proxy-first adsb.lol feed + airplanes.live/direct fallbacks, oriented markers, search/quick filters, list view, flight details, in-session trails, follow mode, route enrichment, and lazy real-aircraft thumbnails from the Planespotters.net Photo API. Photo URLs remain provider-hosted and every photo is linked/credited; missing photos use a SkyLens silhouette. `?icao=<hex>` selects/follows Telegram deep links; `?lat=&lon=&zoom=` shares a map view. **Sky view** uses the rear camera + device orientation/location and degrades to bearing cards where sensors are unavailable. No account required. Setup/test contract: `scripts/SKYLENS-SETUP.md`. |
...
| **Frot deng Daten** | `ask.html`, `supabase/functions/ask` | **AI over the user's own data** (finance/health/workouts/subs/countdowns + Ian's kart sessions). No tool loop — per request the function builds a compact JSON snapshot (service role, always filtered to the caller's uid) and answers in one model call. Routing like study-buddy: Ian → Sonnet 4.6 uncapped, others → Haiku 4.5 sharing the `ai_usage` 40/day cap. Home tile 🧠 (signed-in). |
| **Moies-Briefing** | `supabase/functions/briefing`, `scripts/briefing-v1.sql` | Daily 07:00 push to Ian: weather (Open-Meteo, Niederanven), next 2 countdowns, Shorts posted overnight (`data/factory.json`). NO bus line (Ian's call). Triggered by pg_cron+pg_net (`briefing-v1.sql`, '0 5 * * *' UTC = 07:00 CEST; winter → bump to '0 6'). Guarded by `x-briefing-secret` = `BRIEFING_SECRET` (fails closed); pushes via the shared VAPID keys to `push_subscriptions`. |
| **Lock-screen widget** | `transport` fn `?action=widget`, `scripts/widget-scriptable.js`, `scripts/WIDGET-SETUP.md` | Scriptable widget for Ian's iPhone: 06:50–09:30 the live D02 departure at Laach (stop 200504002, delay/cancelled); rest of the day date+weather in LB. Params `stop/line/from/until` override the defaults; works without `TRANSPORT_API_KEY` (day mode only). |
| **Trails SEO sites (Auto-Pédestres + MTB)** | `trails/**` + `mtb/**` (generated — don't hand-edit), `scripts/trails/{config.py,discover_trails.py,fetch_trails.py,fetch_official.py,route_match.py,fetch_pois.py,enrich.py,composer.py,build.py,build_map.py,strings.py,templates/*}`, `data/trails/*` + `data/mtb/*` (each: registry/curated/computed/pois/affiliate/geo), `sitemap-trails.xml`, `sitemap-mtb.xml` | **Programmatic-SEO directories** (money-plan workstream C): ALL 154 official Auto-Pédestre loop walks at `/trails/` + ALL (~37 usable of 40) marked MTB loops at `/mtb/`. One category-driven pipeline (`config.py`, every script takes `--cat mtb`): `discover_trails.py` (OSM → registry) → `fetch_trails.py` (geometry/bus/region, chunked Overpass) → **`fetch_official.py`** (hiking only: reconciles the registry against geoportail.lu's CC0 layer — the ministry cut the network from 201 circuits to 154, so OSM both misses 66 real ones and still carries 17 retired ones. Matching is by route **overlap** (`route_match.py`), never by name; matched circuits keep their slug/geometry/enrichment, unmatched ones are added from the official line, and published routes that match nothing are marked `"retired": true` and their pages deleted by `build.py`) → `fetch_pois.py` (country POIs, shared) → `enrich.py` (EU-DEM elevation + POIs + CC Commons photos w/ attribution, resumable) → `build.py` (DE/FR/EN static pages: honest `lang`, hreflang, JSON-LD, Leaflet map, SVG elevation profile, photo gallery, GPX download, **Komoot link pre-filled with the whole route as waypoints**, Google-Maps start point, bus stops+lines). `/trails/` + `/mtb/` choosers auto-pick language (navigator.language, lb→de) with sticky localStorage override `trails_lang`; per-lang indexes cross-link the sister site. Texts: per-cat `curated.json` overrides (10 hand-written hiking flagships; MTB uses composed prose + place fixes; `"exclude": true` drops broken OSM routes) else `composer.py` fact-driven variants per category. Sitemaps in robots.txt + index URLs in `sitemap.xml`. `affiliate.json` gear links render only when filled. |
...
| **Meme Party** | `meme.html` | **MakeItMeme-style** realtime party game on `meme:<room>`. Everyone captions the **same** meme template (Impact top/bottom text, live preview), then votes for the funniest; +100/vote over `ROUNDS=4`. **Host-authoritative** phase machine (lobby→caption→vote→reveal→final; 70s/35s timers, early-end when all in, live ready-counter). ~311 templates fetched at runtime (memegen 211 + imgflip 100, both CORS; 48 hardcoded `FALLBACK`). Each player can **reroll their template up to `MAX_REROLLS=10`×** during captioning (per-player templates travel with the caption). Launched from the hub (`openOnline` → `?room=&role=host/guest`) or self-generates a room + copyable invite link. 3+ players best. No SQL (Broadcast only). |
