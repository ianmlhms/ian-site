...
| **Auto-Pédestres (Trails SEO site)** | `trails/**` (generated — don't hand-edit), `scripts/trails/{fetch_trails.py,build.py,strings.py,templates/*}`, `data/trails/{trails.json,computed.json,affiliate.json,geo/*}`, `sitemap-trails.xml` | **Programmatic-SEO directory of Luxembourg's Auto-Pédestre loop walks** (money-plan workstream C). Static DE/FR/EN pages (NO LB — AdSense; honest `lang`, hreflang cluster, JSON-LD), each with Leaflet map (route GeoJSON from OSM Overpass) + "getting there by bus" (nearest stops from `moien-stops.json` + lines from `moien-lines.json`). Edit content in `data/trails/trails.json`, then `python3 scripts/trails/build.py`; new trails: add slug+`osm_rel`, run `fetch_trails.py` first. 10 trails/34 pages live; scale (~105 APs in OSM) only after Search Console shows indexing. `affiliate.json` gear links render only when filled. Sitemaps: `sitemap-trails.xml` (all pages, in robots.txt) + 4 index URLs in `sitemap.xml`. |
