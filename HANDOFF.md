...
| **Trails SEO sites (Auto-Pédestres + MTB)** | `trails/**` + `mtb/**` (generated — don't hand-edit), `scripts/trails/{config.py,discover_trails.py,fetch_trails.py,fetch_pois.py,enrich.py,composer.py,build.py,strings.py,templates/*}`, `data/trails/*` + `data/mtb/*` (each: registry/curated/computed/pois/affiliate/geo), `sitemap-trails.xml`, `sitemap-mtb.xml` | **Programmatic-SEO directories** (money-plan workstream C): ALL ~105 Auto-Pédestre loop walks at `/trails/` + ALL (~37 usable of 40) marked MTB loops at `/mtb/`. One category-driven pipeline (`config.py`, every script takes `--cat mtb`): `discover_trails.py` (OSM → registry) → `fetch_trails.py` (geometry/bus/region, chunked Overpass) → `fetch_pois.py` (country POIs, shared) → `enrich.py` (EU-DEM elevation + POIs + CC Commons photos w/ attribution, resumable) → `build.py` (DE/FR/EN static pages: honest `lang`, hreflang, JSON-LD, Leaflet map, SVG elevation profile, photo gallery, GPX download, **Komoot link pre-filled with the whole route as waypoints**, Google-Maps start point, bus stops+lines). `/trails/` + `/mtb/` choosers auto-pick language (navigator.language, lb→de) with sticky localStorage override `trails_lang`; per-lang indexes cross-link the sister site. Texts: per-cat `curated.json` overrides (10 hand-written hiking flagships; MTB uses composed prose + place fixes; `"exclude": true` drops broken OSM routes) else `composer.py` fact-driven variants per category. Sitemaps in robots.txt + index URLs in `sitemap.xml`. `affiliate.json` gear links render only when filled. |
