...
| **Meme Party** | `meme.html` | **MakeItMeme-style** realtime party game on `meme:<room>`. Everyone captions the **same** meme template (Impact top/bottom text, live preview), then votes for the funniest; +100/vote over `ROUNDS=4`. **Host-authoritative** phase machine (lobby→caption→vote→reveal→final; 70s/35s timers, early-end when all in, live ready-counter). 48 Imgflip template URLs hardcoded in `POOL`. Launched from the hub (`openOnline` → `?room=&role=host/guest`) or self-generates a room + copyable invite link. 3+ players best. No SQL (Broadcast only). |
