...
| **Auto-Pédestres (Trails SEO site)** | `trails/**` (generated — don't hand-edit), `scripts/trails/{discover_trails.py,fetch_trails.py,fetch_pois.py,enrich.py,composer.py,build.py,strings.py,templates/*}`, `data/trails/{registry.json,curated.json,computed.json,pois.json,affiliate.json,geo/*}`, `sitemap-trails.xml` | **Programmatic-SEO directory of ALL ~105 Auto-Pédestre loop walks** (money-plan workstream C). Static DE/FR/EN pages (NO LB — AdSense; honest `lang`, hreflang, JSON-LD), each with Leaflet map (simplified OSM route), **elevation profile SVG + gain** (opentopodata EU-DEM), **real CC-licensed Wikimedia-Commons photos with attribution**, POIs on route, and "getting there by bus" (from `moien-stops/lines.json`). Pipeline: `discover_trails.py` (auto-finds APs in OSM → registry) → `fetch_trails.py` (geometry/bus, chunked) → `fetch_pois.py` (country POIs, once) → `enrich.py` (elevation+POIs+photos, resumable, rate-limited) → `build.py`. Texts: `curated.json` hand-written overrides (10 flagship) else fact-driven composed prose (`composer.py`, deterministic variants). Sitemaps: `sitemap-trails.xml` (in robots.txt) + 4 index URLs in `sitemap.xml`. `affiliate.json` gear links render only when filled. |
