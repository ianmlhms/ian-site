...
import { registerSW, enablePush, disablePush, pushState } from "./notify.js?v=10";
...
const ipTag = (uid) => (ipIds.has(uid) ? ` <span class="ip-tag">💠 IP</span>` : "");
...
let ipIds = new Set();        // user_ids of IP users → shown with a 💠 IP tag
...
async function toggleMembers() {
  ...
  p.querySelector(".mp-list").innerHTML = error || !data ? T("msg.loadFail") :
    data.map((m) => `<div class="mp-row"><span class="mp-av">${avatarHtml(m.user_id)}</span> ${esc(m.username)}${classTag(m.user_id)}${adminTag(m.user_id)}${vipTag(m.user_id)}${ipTag(m.user_id)}${m.user_id === me ? ` <span class='you'>${T("msg.you")}</span>` : ""}</div>`).join("");
}
...
function appendMessage(m) {
  ...
  el.innerHTML = `${avHtml}<div class="bubble"><span class="who">${esc(m.username)}${classTag(m.user_id)}${adminTag(m.user_id)}${vipTag(m.user_id)}${ipTag(m.user_id)}</span>${replyHtml}<span class="msg-text">${body}</span>${m.edited_at ? `<span class="edited">${T("msg.edited")}</span>` : ""}<span class="time">${fmtTime(m.created_at)}</span><span class="reacts"></span></div>`;
  const bubble = el.querySelector(".bubble");
...
async function boot() {
  ...
  try { const { data } = await sb.rpc("ip_user_ids"); ipIds = new Set((data || []).map((r) => r.user_id)); } catch (e) { console.warn("[msgr] ip ids", e); }
