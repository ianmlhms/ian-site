...
let ipIds = new Set();       // user_ids of IP users → 💠 IP tag
...
function renderFriends(list) {
  ...
  el.innerHTML = ordered.map(f => `
    <div class="row">
      <span class="name"><span class="av">👤</span>${esc(f.username)}${classTag(f.username)}${adminIds.has(f.user_id) ? ` <span class="admin-tag">👑 Admin</span>` : ""}${vipTag(f.user_id)}${ipTag(f.user_id)}</span>
...
(async function boot() {
  ...
  try { const { data } = await sb.rpc("ip_user_ids"); ipIds = new Set((data || []).map((r) => r.user_id)); } catch (e) { console.warn("[friends] ip ids", e); }
